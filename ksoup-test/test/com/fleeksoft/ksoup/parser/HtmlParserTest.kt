...
class HtmlParserTest {
    ...
    fun dropsDuplicateAttributes() {
        parameterizedTest(dupeAttributeData()) {
            ...
            assertEquals("Dropped duplicate attribute(s) in tag [$tag]", parser.getErrors()[0].errorMsg)
        }
    }
    ...
    fun dropsUnterminatedAttribute() {
        // ksoup used to parse this to <p id="foo">, but whatwg, webkit will drop.
        val h1 = "<p id=\"foo"
    ...
    fun createsDocumentStructure() {
        val html = "<meta name=keywords /><link rel=stylesheet /><title>ksoup</title><p>Hello world</p>"
        val doc = Ksoup.parse(html)
        val head = doc.head()
        val body = doc.body()
    ...
    fun doesNotCreateImplicitLists() {
        // old ksoup used to wrap this in <ul>, but that's not to spec
    ...
    fun discardsNakedTds() {
        // ksoup used to make this into an implicit table; but browsers make it into a text run
    ...
    fun handlesUnclosedCdataAtEOF() {
        // would crash, as character reader would try to seek past EOF
        val h = "<![CDATA[]]"
        val doc = Ksoup.parse(h)
    ...
    fun handlesUnclosedDefinitionLists() {
        // ksoup used to create a <dl>, but that's not to spec
    ...
    fun testNoImagesInNoScriptInHead() {
        // ksoup used to allow, but against spec if parsing with noscript
    ...
    fun handlesUnclosedScriptAtEof() {
        assertEquals(
            "Data",
            Ksoup.parse("<script>Data").select("script").first()!!.data(),
        )
        assertEquals(
            "Data<",
            Ksoup.parse("<script>Data<").select("script").first()!!.data(),
        ...
        assertEquals(
            "Data</sc",
            Ksoup.parse("<script>Data</sc").select("script").first()!!.data(),
        )
        assertEquals(
            "Data</-sc",
            Ksoup.parse("<script>Data</-sc").select("script").first()!!.data(),
        ...
        assertEquals(
            "Data",
            Ksoup.parse("<script>Data</script>").select("script").first()!!.data(),
        )
        assertEquals(
            "Data</script",
            Ksoup.parse("<script>Data</script").select("script").first()!!.data(),
        ...
        assertEquals(
            "Data",
            Ksoup.parse("<script>Data</script n").select("script").first()!!.data(),
        )
        assertEquals(
            "Data",
            Ksoup.parse("<script>Data</script n=").select("script").first()!!.data(),
        ...
        assertEquals(
            "Data",
            Ksoup.parse("<script>Data</script n=\"p").select("script").first()!!.data(),
        )
    }
    ...
    fun handlesUnclosedRawtextAtEof() {
        assertEquals(
            "Data",
            Ksoup.parse("<style>Data").select("style").first()!!.data(),
        )
        assertEquals(
            "Data</st",
            Ksoup.parse("<style>Data</st").select("style").first()!!.data(),
        )
        assertEquals(
            "Data",
            Ksoup.parse("<style>Data</style>").select("style").first()!!.data(),
        )
        assertEquals(
            "Data</style",
            Ksoup.parse("<style>Data</style").select("style").first()!!.data(),
    ...
    fun noImplicitFormForTextAreas() {
        // old ksoup parser would create implicit forms for form children like <textarea>, but no more
    ...
    fun tracksErrorsWhenRequested() {
        ...
        assertEquals(
            "<3:36>: Invalid character reference: character [1114112] outside of valid range",
            errors[6].toString(),
        )
  ...
  // -->
    ...
    fun handlesTagsInTextarea() {
        val html = "<textarea><p>Ksoup</p></textarea>"
        val doc = Ksoup.parse(html)
        assertEquals("<textarea>&lt;p&gt;Ksoup&lt;/p&gt;</textarea>", doc.body().html())
    }
...
<input type="hidden" name="b" value="" />
    ...
    fun fallbackToUtfIfCantEncode() {
        ...
        val doc = Ksoup.parseInput(input = input.byteInputStream(), baseUri = "", charsetName = null)
        ...
        assertEquals(
            "<html><head><meta charset=\"ISO-SSS\"></head><body>One</body></html>",
 ...
 <p>2</p>
        ...
        val caseDoc =
            Ksoup.parse(html = html, baseUri = "", parser = Parser.htmlParser().settings(ParseSettings.preserveCase))
        assertEquals(
            """<body>
 <p>1</p>
 <P>2</P>
  ...
  <option>six
    ...
    @Test
    fun preservesTabs() {
        // testcase to demonstrate tab retention
    ...
    @Test
    fun testNoErrorOnEofIfBodyOpen() {
        val html = "<body>"
        val parser = Parser.htmlParser()
        parser.setTrackErrors(10)
        Ksoup.parse(html, parser)
 ...
 <foreignObject>
  <p>One</p>
    ...
    fun tableInPInQuirksMode() {
        ...
        html =
            "<!DOCTYPE html><p><span><table><tbody><tr><td><span>Hello table data</span></td></tr></tbody></table></span></p>"
        doc = Ksoup.parse(html)
