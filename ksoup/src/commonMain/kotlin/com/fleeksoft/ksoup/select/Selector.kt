 ...
 * <tr><td>`:is(*selector list*)`</td><td>elements that match any of the selectors in the selector list</td>`:is(h1, h2, h3, h4, h5, h6)` finds any heading element.<br></br>`:is(section, article) > :is(h1, h2)` finds a `h1` or `h2` that is a direct child of a `section` or an `article`</tr>
 ...
 *
 *To find content that includes parentheses, escape those with a `\`.</td><td>`p:contains(com.fleeksoft.ksoup)` finds p elements containing the text "com.fleeksoft.ksoup".
 *
 *`p:contains(hello \(there\) finds p elements containing the text "Hello (There)"`</td></tr>
 * <tr><td>`:containsOwn(*text*)`</td><td>elements that directly contain the specified text. The search is case insensitive. The text must appear in the found element, not any of its descendants.</td><td>`p:containsOwn(com.fleeksoft.ksoup)` finds p elements with own text "com.fleeksoft.ksoup".</td></tr>
 * <tr><td>`:containsData(*data*)`</td><td>elements that contains the specified *data*. The contents of `script` and `style` elements, and `comment` nodes (etc) are considered data nodes, not text nodes. The search is case insensitive. The data may appear in the found element, or any of its descendants.</td><td>`script:contains(com.fleeksoft.ksoup)` finds script elements containing the data "com.fleeksoft.ksoup".</td></tr>
 ...
 *
 *To find content that includes parentheses, escape those with a `\`.</td><td>`p:containsWholeText(com.fleeksoft.ksoup\nThe Java HTML Parser)` finds p elements containing the text `"com.fleeksoft.ksoup\nThe Java HTML Parser"` (and not other variations of whitespace or casing, as `:contains()` would. Note that `br` elements are presented as a newline.</td></tr>
 ...
 *
 *To find content that includes parentheses, escape those with a `\`.</td><td>`p:containsWholeOwnText(com.fleeksoft.ksoup\nThe Java HTML Parser)` finds p elements directly containing the text `"com.fleeksoft.ksoup\nThe Java HTML Parser"` (and not other variations of whitespace or casing, as `:contains()` would. Note that `br` elements are presented as a newline.</td></tr>
 ...
 *
 ***Note** that using this selector will modify the DOM, so you may want to `clone` your document before using.</td><td>`p:matchText:firstChild` with input `<p>One<br />Two</p>` will return one [com.fleeksoft.ksoup.nodes.PseudoTextElement] with text "`One`".</td></tr>
