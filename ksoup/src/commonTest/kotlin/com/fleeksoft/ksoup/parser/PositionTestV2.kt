...
class PositionTestV2 {
    ...
    fun tracksPosition() {
        val html = "<p id=1\n class=foo>\n<span>Hello\n &reg;\n there &copy.</span> now.\n <!-- comment --> "
        val doc: Document = Ksoup.parse(html, TrackingParser)
    ...
    fun tracksXml() {
        ...
        val doc: Document = Ksoup.parse(xml, Parser.xmlParser().setTrackPosition(true))
    ...
    fun tracksTableMovedText() {
        ...
        val textNodes: List<TextNode> =
            doc.nodeStream(TextNode::class).toList()
    ...
    companion object {
        var TrackingParser: Parser = Parser.htmlParser().setTrackPosition(true)
    }
}
