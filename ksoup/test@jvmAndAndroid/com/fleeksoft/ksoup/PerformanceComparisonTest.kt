...
class PerformanceComparisonTest {
    ...
    fun compareWithJsoup() = runTest {
        ...
        val testData = File("/Users/sabeeh/IdeaProjects/ksoup-new/ksoup-test/testResources/test.txt").readBytes().decodeToString()
//        val testData = TestHelper.getResourceAbsolutePath("test.txt").uniVfs.readString()
        ...
        val selectTime = measureTimeMillis {
            ...
            doc.getElementsByClass("an-info").mapNotNull { anInfo ->
                anInfo.parent()?.let { a ->
                    ...
                    attr.substringAfter("/Home/Bangumi/", "").takeIf { it.isNotBlank() }
                }
            }
        }
    ...
    private fun printComparisonTable(
        ksoupParseTimes: List<Long>, ksoupSelectTimes: List<Long>, jsoupParseTimes: List<Long>, jsoupSelectTimes: List<Long>
    ) {
